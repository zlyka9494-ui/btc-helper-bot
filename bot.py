...
async def start(message: types.Message):
    await message.answer(
        "👋 Добро пожаловать.\n\n"
        "Я помогу быстро обменять рубли на Bitcoin.\n\n"
        "Что хотите сделать?",
...
async def done_order(callback: types.CallbackQuery):
    ...
    if order:
        await bot.send_message(
            order["user_id"],
            "🎉 Готово!\n\n"
            "Bitcoin отправлены на ваш кошелёк.\n\n"
            "Спасибо, что воспользовались сервисом ❤️"
        )
...
async def cancel_order(callback: types.CallbackQuery):
    ...
    if order:
        await bot.send_message(
            order["user_id"],
            f"❌ Операция BTC-{oid} отменена.\n\n"
            "Если нужна помощь, напишите личному помощнику."
        )
...
async def text_handler(message: types.Message):
    ...
    if state and state.get("step") == "admin_requisites":
        oid = state["order_id"]
        order = orders.get(oid)
